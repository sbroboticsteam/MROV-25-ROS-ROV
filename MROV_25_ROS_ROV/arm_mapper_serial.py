...
class ArmMapperSerial(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info("Thruster Mapper Serial Node Ready")
    ...
    def callback(self, msg: Float32MultiArray):
        ...
        serial_str = ",".join(str(v) for v in mapped) + "\n"

        try:
            self.ser.write(serial_str.encode())
        except Exception as e:
            self.get_logger().errors(f"Serial write failed: {e}")

        # Debug
        self.get_logger().info(
            "Sent: " + ", ".join(str(v) for v in mapped)
        )
